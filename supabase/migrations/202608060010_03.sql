-- Update existing student IDs to CC prefix
UPDATE public.profiles
SET student_id = REPLACE(student_id, 'KC', 'CC')
WHERE student_id LIKE 'KC%';

-- Update in applications table as well, in case they differ
UPDATE public.applications
SET application_number = REPLACE(application_number, 'KC', 'CC')
WHERE application_number LIKE 'KC%';

-- Assign student IDs to profiles that don't have one
UPDATE public.profiles
SET student_id = 'CC' || LPAD(FLOOR(RANDOM() * 10000000)::TEXT, 7, '0')
...
SET application_number = COALESCE(
    ...
    'CC' || LPAD(FLOOR(RANDOM() * 10000000)::TEXT, 7, '0')
)
