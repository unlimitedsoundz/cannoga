-- Comprehensive fix for Subject records with wrong IDs and names
-- Issues found:
-- 1. IDs containing dots (e.g., ARCHM.AR001)
-- 2. Names with embedded course codes (e.g., "BACHEL 101: Data Structures and Algorithms")
-- 3. Names with leading/trailing spaces
-- 4. Duplicate names across different subject IDs
-- 5. Codes not matching the cleaned names

-- Step 1: Fix IDs containing dots
UPDATE public."Subject"
SET id = REPLACE(id, '.', '_')
WHERE id LIKE '%.%';

-- Step 2: Remove embedded course codes from names
-- Pattern: "PREFIX 1234: Actual Title" -> "Actual Title"
UPDATE public."Subject"
SET name = TRIM(REGEXP_REPLACE(name, '^[A-Z]{2,6}\s*\d{3,4}:\s*', ''))
WHERE name ~ '^[A-Z]{2,6}\s*\d{3,4}:';

-- Step 3: Clean up leading/trailing spaces and double spaces
UPDATE public."Subject"
SET name = TRIM(REGEXP_REPLACE(name, '\s+', ' '))
WHERE name LIKE '%  %' OR name LIKE ' %' OR name LIKE '% ';

-- Step 4: Fix duplicate names by making them unique with context
...
-- Step 5: Regenerate codes for all subjects to ensure consistency
-- This will use the same logic as the previous code generation but on cleaned names
