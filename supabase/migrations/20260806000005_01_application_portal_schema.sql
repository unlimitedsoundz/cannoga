...
-- 1. PROCALES & ROLES
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'user_role') THEN CREATE TYPE user_role AS ENUM ('APPLICANT', 'ADMISSIONS', 'ADMIN'); END IF; END $$;
...
-- 2. APPLICATIONS
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'application_status') THEN CREATE TYPE application_status AS ENUM ('DRAFT', 'SUBMITTED', 'UNDER_REVIEW', 'DOCS_REQUIRED', 'ADMITTED', 'REJECTED', 'OFFER_ACCEPTED', 'OFFER_DECLINED', 'ENROLLED'); END IF; END $$;
...
-- 3. DOCUMENTS
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'document_type') THEN CREATE TYPE document_type AS ENUM ('PASSPORT', 'TRANSCRIPT', 'CERTIFICATE', 'CV', 'MOTIVATION_LETTER', 'LANGUAGE_CERT'); END IF; END $$;
...
DROP POLICY IF EXISTS "Applicants can view own docs" ON public.application_documents;
...
DROP POLICY IF EXISTS "Applicants can upload docs to own application" ON public.application_documents;
...
-- 4. ADMISSION OFFERS
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'offer_status') THEN CREATE TYPE offer_status AS ENUM ('PENDING', 'ACCEPTED', 'DECLINED'); END IF; END $$;
...
DROP POLICY IF EXISTS "Applicants can view own offers" ON public.admission_offers;
...
-- 5. TUITION PAYMENTS
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'payment_status') THEN CREATE TYPE payment_status AS ENUM ('INITIATED', 'PROCESSING', 'COMPLETED', 'FAILED'); END IF; END $$;
...
DROP POLICY IF EXISTS "Applicants can view own payments" ON public.tuition_payments;
