  ...
  const columns = [
    { key: 'code', header: 'Section', render: (s: any) => (
      <div>
        ...
        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{s.module?.code} - {s.module?.title}</div>
      </div>
    )},
    ...
    { key: 'actions', header: 'Actions', render: (s: any) => (
      <div className="flex justify-end gap-2">
        <button onClick={(e) => { e.stopPropagation(); openMeetingsPanel(s.id); }} className="p-1.5 hover:bg-neutral-100 rounded text-slate-400 hover:text-black" title="Manage meetings">
          <HugeiconsIcon icon={Calendar} size={14} strokeWidth={2.5} />
        </button>
        <button onClick={(e) => { e.stopPropagation(); openSectionModal(s); }} className="p-1.5 hover:bg-neutral-100 rounded text-slate-400 hover:text-black">
          <HugeiconsIcon icon={Settings} size={14} strokeWidth={2.5} />
        </button>
        <button onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ type: 'section', id: s.id }); }} className="p-1.5 hover:bg-neutral-100 rounded text-slate-400 hover:text-red-600">
  ...
  const meetingColumns = [
    ...
    { key: 'actions', header: 'Actions', render: (m: CourseSectionMeeting) => (
      <div className="flex justify-end">
        <button onClick={() => handleDeleteMeeting(m.id)} className="p-1.5 hover:bg-neutral-100 rounded text-slate-400 hover:text-red-600">
          <HugeiconsIcon icon={Trash} size={14} strokeWidth={2.5} />
        </button>
      </div>
    )},
