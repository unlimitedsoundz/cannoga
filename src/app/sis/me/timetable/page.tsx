...
import { Calendar, DownloadSimple as Download, MapPin, VideoCamera as Video, Clock, CaretLeft as ChevronLeft, CaretRight as ChevronRight } from '@phosphor-icons/react';
  ...
  const fetchTimetable = async () => {
    try {
      ...
      const { data } = await supabase
        ...
        .select(`
          *,
          section:course_sections(session_type, delivery_mode, module:modules(id, code, title, credits), instructor_id),
                    ...
                    return (
                      <div key={dayIndex} className="p-1 border-r last:border-r-0 border-neutral-100 flex flex-col gap-1 relative">
                        {daySessions.map(session => (
                          <div key={session.id} className={`p-2 rounded-sm border-l-4 shadow-sm transition-all hover:scale-[1.02] cursor-default
