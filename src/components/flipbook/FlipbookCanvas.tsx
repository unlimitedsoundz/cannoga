...
export const FlipbookCanvas = forwardRef<FlipbookCanvasHandle, FlipbookCanvasProps>(
    function FlipbookCanvas(
        { pages, initialPage = 1, viewMode = 'spread', onPageChange, onOrientationChange, onReady },
        ref
    ) {
        ...
        const [currentOrientation, setCurrentOrientation] = useState<FlipOrientation>(
            viewMode === 'single' ? 'portrait' : 'landscape'
        );
        ...
        useImperativeHandle(ref, () => ({
            ...
            getOrientation: () => {
                if (!pageFlipInstance.current) return viewMode === 'single' ? 'portrait' : 'landscape';
        ...
        useEffect(() => {
            ...
            pages.forEach((page) => {
                ...
                pageItem.className = `flipbook-page-item overflow-hidden bg-[#0a151a] relative ${
                    isCover && !isSingleMode ? '--hard --cover-front' : ''
                } ${isBackCover && !isSingleMode ? '--hard --cover-back' : ''}`;
                pageItem.setAttribute('data-density', (!isSingleMode && (isCover || isBackCover)) ? 'hard' : 'soft');
                ...
                // Spine shadow (only in 2-page spread mode)
                if (!isSingleMode && !isCover && !isBackCover) {
            ...
            try {
                const flip = new PageFlip(bookEl, {
                    ...
                    size: 'stretch',
                    minWidth: 260,
                    maxWidth: isSingleMode ? 640 : 750,
                    minHeight: 260,
                    maxHeight: isSingleMode ? 640 : 750,
                    maxShadowOpacity: isSingleMode ? 0.2 : 0.45,
                ...
                const updateState = () => {
                    ...
                    const orient = isSingleMode ? 'portrait' : (rawOrient === 'portrait' ? 'portrait' : 'landscape');
                    ...
                    if (orient === 'landscape' && !isSingleMode) {
                        if (currentIdx === 0) {
                            spread = [1];
                ...
                flip.on('changeOrientation', (e: { data: string }) => {
                    const orient = isSingleMode ? 'portrait' : (e.data === 'portrait' ? 'portrait' : 'landscape');
                    setCurrentOrientation(orient);
                    onOrientationChangeRef.current(orient);
                    updateState();
                });
        ...
        }, [isMounted, pages, initialPage, viewMode]);
        ...
        let coverOffset = '0%';
        if (!isSingleMode && currentOrientation === 'landscape') {
            if (isFrontCover) {
                coverOffset = '-25%';
            } else if (isBackCover) {
                coverOffset = '25%';
